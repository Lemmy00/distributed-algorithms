...
private:
    ...
    std::unordered_map<uint32_t, std::unordered_set<uint16_t>> proposals;
    std::unordered_map<uint32_t, std::unordered_set<uint16_t>> accepted_values;
    ...
    std::unordered_map<uint32_t, uint8_t> num_nacks;
    ...
    const uint32_t sequence_buffer_max_latency = 100;
...
public:
    ...
    void propose(const std::unordered_set<uint16_t> &proposal, uint32_t seq_num);
...
private:
    ...
    static void set_union(std::unordered_set<uint16_t> &a, const std::unordered_set<uint16_t> &b);
    static bool is_subset_of(const std::unordered_set<uint16_t> &a, const std::unordered_set<uint16_t> &b);
};
...
void LatticeAgreement::propose(const std::unordered_set<uint16_t> &proposal, uint32_t seq_num)
{
    {
...
void LatticeAgreement::handleAck(const ProposalMessage &proposal, std::function<void(const std::string &)> deliverCallback)
{
    ...
    if (num_acks[proposal.getSeqNum()] > threshold_acks)
    {
        ...
        // try to deliver
        {
            ...
            {
                std::cout << "Delivering proposal " << it->first << ": " << it->second << std::endl;
...
void LatticeAgreement::set_union(std::unordered_set<uint16_t> &a, const std::unordered_set<uint16_t> &b)
{
...
bool LatticeAgreement::is_subset_of(const std::unordered_set<uint16_t> &a, const std::unordered_set<uint16_t> &b)
{
