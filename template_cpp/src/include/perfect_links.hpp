...
public:
    PerfectLinks(uint8_t sender_id, in_addr_t ip, unsigned short port, std::function<void(const MessageBatch &)> deliverCallback);
    ~PerfectLinks();
...
PerfectLinks::PerfectLinks(uint8_t sender_id, in_addr_t ip, unsigned short port, std::function<void(const MessageBatch &)> deliverCallback)
...
void PerfectLinks::receiverWorker()
{
    while (!stopThreads)
    {
        ...
        if (recv_size > 0)
        {
            ...
            if (msgBatch.get_is_ack())
            {
                std::lock_guard<std::mutex> lock(ackedMessagesMutex);
                // std::cout << "Received ACK for message with ID: " << msgBatch.get_sender_id() << ":" << msgBatch.get_batch_key().first << ":" << msgBatch.get_batch_key().second << "\n";
                ackedMessages[msgBatch.get_sender_id()].insert(msgBatch.get_batch_key());
                continue;
            }

            MessageBatch ack_msgs(msgBatch.get_batch_key(), this->sender_id, msgBatch.get_sender_id(), src_addr.s_addr, src_port, true);
            sendAck(ack_msgs);

            {
                std::lock_guard<std::mutex> lock(receivedMessagesMutex);
                auto it = receivedMessages.find(msgBatch.get_sender_id());
                if (it != receivedMessages.end() && it->second.find(msgBatch.get_batch_key()) != it->second.end())
                {
                    continue;
                }

                receivedMessages[msgBatch.get_sender_id()].insert(msgBatch.get_batch_key());
            }
